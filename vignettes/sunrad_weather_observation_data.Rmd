---
title: "fmi2: introductionPJA"
author: "Joona Lehtomäki (revised by Pedro J. Aphalo)"
date: "`r Sys.Date()`"
output: 
  prettydoc::html_pretty:
    ...
    highlight: github
vignette: >
  %\VignetteIndexEntry{fmi2 introductionPJA}
...
Let's plot the daily temperature data for the different locations as lines.
...
Let's plot the daily temperature data for the different locations as a line for
mean and a band for the range.
...
Sun radiation data is available only for a few stattions: 
...
A place name as a character string is also accepted (not run here).

```{r getting-sun-data-place, eval=FALSE}
# Use Kumpula station FMISID
kumpula_sun_data <- obs_sunrad_minute(starttime = "2019-01-01",
                                      endtime = "2019-01-02",
                                      place = "Kumpula")
...
Haven't found any station returning soil data!! (Not run)

```{r getting-soil-data, eval=FALSE}
# Use Kumpula station FMISID
...
For finding stored queries available from the server we first need to download 
...
Stored queries with ids ending in `::simple` return data in simple feature 
(`sf`) format, and are likely to be supported `fmi_obs_simple()`. For queries
returning data in other formats, there is not yet support in this package.
...
example `"radiation"` is used for sun radiation in the `"fmi"` queries but
for other radiation in the `"stuk"` queries.
